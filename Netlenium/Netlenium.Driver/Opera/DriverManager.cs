...
namespace Netlenium.Driver.Opera
{
    ...
    public class DriverManager : IDriverManager
    {
        ...
        public Version GetLatestVersion()
        {
            try
            {
                driver.Logging.WriteEntry(MessageType.Verbose, "DriverManager", "Creating HTTP/S Request to GitHub API For the latest Release on 'operasoftware/operachromiumdriver'");

                var release = WebAPI.GitHub.Releases.GetLatestRelease("operasoftware", "operachromiumdriver");
                ...
                driver.Logging.WriteEntry(MessageType.Debugging, "DriverManager", $"Published At => {release.PublishedAt}");
                driver.Logging.WriteEntry(MessageType.Debugging, "DriverManager", $"Release URL => {release.HtmlUrl}");

                return new Version(release.TagName.Substring(2));
            }
            catch (Exception e)
            {
